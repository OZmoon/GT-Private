				...
				if (member_search->display_name == "") {
					member_search->display_name = "`o" + member_search->member_name;
						...
						if (ifs.is_open()) {
							json j;
							ifs >> j;
							if (j["`#@"] == 1) member_search->display_name = "`#@" + member_search->member_name;
							if (j["`9@"] == 1) member_search->display_name = "`9@" + member_search->member_name;
							if (j["`6@"] == 1) member_search->display_name = "`6@" + member_search->member_name;
							if (j["`4@"] == 1) member_search->display_name = "`4@" + member_search->member_name;
							if (j["god"] == 1) member_search->display_name = "`8@" + member_search->member_name;
						}
				}
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			for (GuildMember search_member : guild_information->guild_members) {
				if (search_member.member_name == pInfo(currentPeer)->tankIDName) {
					string display_color = (pInfo(currentPeer)->superdev == 1 ? "`6@" : pInfo(currentPeer)->god == 1 ? "`8@" : pInfo(currentPeer)->dev == 1 ? "`9@" : pInfo(currentPeer)->promo == 1 ? "`4@" : (pInfo(currentPeer)->tmod == 1) ? "`#@" : "`o");
					...
					if (search_member->display_name == "") {
						search_member->display_name = "`o" + search_member->member_name;
							...
							if (ifs.is_open()) {
								...
								if (j["superdev"] == 1) search_member->display_name = "`6@" + search_member->member_name;
								if (j["promo"] == 1) search_member->display_name = "`6@" + search_member->member_name;
								if (j["god"] == 1) search_member->display_name = "`8@" + search_member->member_name;
							}
					}
